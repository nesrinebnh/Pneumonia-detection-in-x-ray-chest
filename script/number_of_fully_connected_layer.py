#please to find the chart of this graph in charts\comparisoncomparing_the_variation_of_validation_accuracy_in_function_of_epochs_by_using_different_fully_connected_layers.PNG
...
for j in range(nets):
	...
	if j>0:
		model[j].add(Dense(units = 128 , activation = 'relu'))
	if j>1:
		model[j].add(Dense(units = 64 , activation = 'relu'))
...
utils.plot_muliplt_line_chart(plt, 'comparing_the_variation_of_validation_accuracy_in_function_of_epochs_by_using_different_fully_connected_layers', legend_names)